...
- **Render cycle:** every change updates `state` or `ui` and then calls `renderBoard()`. It is the only function that writes card HTML: it runs `applyFilters()`, builds each column from `renderCard()` strings, and calls `renderSummary()`.
  ...
  - **v2:** status colours live in `--s-done`, `--s-progress`, `--s-blocked` and `--s-backlog`, applied through the `STATUS_CLASS` classes. They were validated as a colour-blind-safe set with every pair checked, not only neighbours in `CHART_ORDER`, because a project bar with no Blocked tasks puts In Progress next to Backlog. Dark mode overrides `--s-done` and `--s-progress`. In light mode the pale Backlog fill gets a 1px edge from `--s-backlog-edge`. Re-validate all pairs in both themes if you change them.
